...
| 4.11 | 🔴 | Exporter (CSV puis PDF) la liste des clients | Segmentation en **libellé** (jamais un nombre) ; colonnes E-mail et Tournée présentes ; pas de colonne Actions ni Statut. Le PDF est un **vrai document** (JasperReports) ouvert dans un nouvel onglet : en-tête société (logo/adresse/tél/site), bandeau de colonnes bleu répété à chaque page, lignes alternées, rappel des filtres sous le titre, « Page X / Y » et total en fin | | |
| 4.11b | 🟠 | Déposer un `clients.jrxml` modifié dans le répertoire des rapports (**D:\REPORTS** par défaut) | L'impression suivante utilise le modèle personnalisé, **sans redéploiement** ; supprimer le fichier fait revenir au modèle embarqué | | |
| 4.11c | 🟠 | Démarrer l'application avec un répertoire D:\REPORTS vide | Les **18 modèles .jrxml** y sont déposés automatiquement ; un fichier déjà présent n'est **jamais écrasé** | | |
| 4.11d | 🟠 | Exporter → PDF depuis Campagnes, Catalogue, Promotions, Disponibilités, Informations, Historique des envois, Recouvrement (agences / encours / historique), Utilisateurs, Connexions, Journal, Tableau de bord (Évolution) | Chaque écran produit une édition JasperReports avec l'en-tête société — même présentation que les comptes clients | | |
| 4.11e | 🔴 | Recouvrement → Relevé de compte d'un client | Même contenu qu'avant (client, situation, factures/avoirs, règlements) mais en édition JasperReports : logo + adresse société en tête, « Page X / Y » ; un client sans créance affiche « Aucune créance enregistrée » | | |
| 4.18 | 🟠 | Assistant d'import : fichier CSV **ANSI** (export Excel avec accents) | Les intitulés s'affichent **sans « � »** (« Numéro client » lisible) et le pré-mapping les reconnaît | | |
| 4.19 | 🔴 | Assistant d'import : fichier avec MOINS de colonnes que l'application | On coche uniquement les champs à récupérer ; les champs **obligatoires (*)** sont cochés d'office et bloquent le lancement tant que leur colonne n'est pas choisie (message listant les manquants) | | |
| 4.20 | 🟠 | Mise à jour sélective : volet « Nouvelles valeurs » | Affiché **à droite** de la grille : les 4 champs, le compteur de comptes cochés et le bouton Appliquer restent visibles sans défilement | | |
