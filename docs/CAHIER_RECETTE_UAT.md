...
| 4.11b | 🟠 | Déposer un `clients.jrxml` modifié dans le répertoire « Modèles de rapport » (Paramètres → Général) | L'impression suivante utilise le modèle personnalisé, **sans redéploiement** ; supprimer le fichier fait revenir au modèle embarqué | | |
